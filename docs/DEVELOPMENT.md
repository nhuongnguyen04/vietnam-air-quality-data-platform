...
At minimum, verify the ClickHouse credentials and add the variables that `docker-compose.yml` expects but `.env.example` does not currently list: `AIRFLOW_ADMIN_USERNAME`, `AIRFLOW_ADMIN_PASSWORD`, `TOMTOM_API_KEY`, `GDRIVE_ROOT_FOLDER_ID`, `GDRIVE_CLIENT_ID`, `GDRIVE_CLIENT_SECRET`, and `GDRIVE_REFRESH_TOKEN`. See [CONFIGURATION.md](./CONFIGURATION.md) for the full inventory.
...
| `cd dbt/dbt_tranform && dbt run --select +mart_air_quality__dashboard --target dev` | Run the dashboard dependency graph used in CI validation. |
...
- **Current CI behavior**: `.github/workflows/ci.yml` runs both `ruff` and `sqlfluff`, but each command ends with `|| true`. That means lint findings are surfaced in CI output without failing the `lint` job.
...
- Expect the CI workflow in `.github/workflows/ci.yml` to execute jobs in this order: `lint` -> `python-unit` -> `python-integration` -> `compile` -> `validate` -> `test`.
...
- Because the `lint` job is currently non-blocking, reviewers should treat lint annotations as real work items even when the workflow stays green.
