"""
AI Assistant page.
Combines Automated AI Analysis and natural language Text-to-SQL Ask Data.
"""
...
from lib.data_service import collect_analysis_context, generate_ai_analysis
from lib.ui_components import render_ai_analysis_panel
...
def _ensure_state() -> None:
    ...
    st.session_state.setdefault("ai_analysis_requested", False)
    st.session_state.setdefault("last_ai_params_hash", None)
...
def _apply_suggestion(value: str) -> None:
...
@page_wrapper("ai_assistant", "Trợ lý AI", icon="🧠")
def main(lang: str):
    _ensure_state()
    
    # Render two tabs: 🧠 Trợ lý phân tích and 💬 Hỏi dữ liệu
    tab_analysis, tab_ask_data = st.tabs([
        t("ai_assistant_tab_analysis", lang),
        t("ai_assistant_tab_ask_data", lang)
    ])
    
    # ── TAB 1: 🧠 Trợ lý phân tích ──
    with tab_analysis:
        # User must select scope via filters and a selectbox
        filters = render_top_filters()
        render_section_divider()
        
        col_scope, col_btn = st.columns([2.0, 1.0], vertical_alignment="bottom")
        with col_scope:
            scope_options = {
                "overview": t("nav_overview", lang),
                "pollutants": t("nav_pollutants", lang),
                "weather": t("nav_weather", lang),
                "traffic": t("nav_traffic", lang),
                "health_risk": t("nav_health", lang),
                "source_comparison": t("nav_comparison", lang),
                "historical_trend": t("nav_trends", lang),
                "alerts": t("nav_alerts", lang),
                "status": t("nav_status", lang),
            }
            selected_scope = st.selectbox(
                t("ai_assistant_scope_label", lang),
                options=list(scope_options.keys()),
                format_func=lambda x: scope_options[x],
                key="ai_analysis_scope_select"
            )
            
        # Monitor scope/filter changes to reset request state
        current_params = {
            "scope": selected_scope,
            "filters": filters
        }
        params_hash = hashlib.md5(json.dumps(current_params, default=str).encode()).hexdigest()
        if st.session_state.get("last_ai_params_hash") != params_hash:
            st.session_state.ai_analysis_requested = False
            st.session_state.last_ai_params_hash = params_hash

        with col_btn:
            if st.session_state.get("ai_analysis_requested", False):
                btn_label = "🔄 Phân tích lại" if lang == "vi" else "🔄 Recalculate"
            else:
                btn_label = "🚀 Phân tích" if lang == "vi" else "🚀 Analyze"
                
            run_analysis = st.button(
                btn_label,
                key="btn_ai_analysis_assistant",
                use_container_width=True
            )
            
        if run_analysis:
            if st.session_state.get("ai_analysis_requested", False):
                generate_ai_analysis.clear()
            st.session_state.ai_analysis_requested = True
            st.rerun()

        if not st.session_state.get("ai_analysis_requested", False):
            st.markdown(f"""
            <div class="glass-card" style="border-left: 5px solid #0891B2; background: rgba(8, 145, 178, 0.04); margin-top: 1rem; padding: 1.2rem;">
                <h4 style="margin:0 0 0.5rem 0; font-family:'Outfit'; color: #0891B2;">🧠 {"Trợ lý phân tích sẵn sàng" if lang == "vi" else "AI Analyst Ready"}</h4>
                <p style="margin:0; font-size:0.9rem; opacity:0.85; line-height:1.5;">
                    {"Chọn phạm vi phân tích và bộ lọc ở trên, sau đó bấm nút <b>Phân tích</b> để bắt đầu nhận phân tích từ trợ lý AI." if lang == "vi" else "Select the analysis scope and filters above, then click the <b>Analyze</b> button to generate insights."}
                </p>
            </div>
            """, unsafe_allow_html=True)
        else:
            # Collect and generate analysis
    ...
    with tab_ask_data:
        ...
        if is_experience_mode:
            ...
            # Custom styling for premium look & layout
            st.markdown("""
            <style>
            ...
            # Two-Column layout within Tab 2
            col_left, col_right = st.columns([0.45, 0.55], gap="large")

            # Left Column: Inputs & Quick Suggestions
            with col_left:
            ...
            # Right Column: Preview SQL & Run Results
            with col_right:
