...
def fetch_one(location_id: str, client: httpx.Client) -> tuple:
    ...
    try:
        ...
        return location_id, LocationData(
            station_name="Unknown", aqi=0,
            success=False, error=str(e), raw_payload=str(e)
        )
...
def scrape_urls_sync(url_paths: List[str], workers: int = WORKERS, progress_callback=None) -> List[tuple]:
    ...
    with httpx.Client(http2=True, timeout=REQUEST_TIMEOUT) as client:
        with ThreadPoolExecutor(max_workers=workers) as executor:
            futures = [executor.submit(fetch_one, path, client) for path in url_paths]
...
def scrape_in_batches(url_paths: List[str], batch_size: int = 100, batch_delay: float = 0.5, workers: int = WORKERS, progress_callback=None) -> List[tuple]:
    all_results = []
